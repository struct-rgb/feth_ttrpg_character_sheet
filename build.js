...
function loadDefinitions(tbase, compiled, omit) {
	...
	for (let namespace of namespaces) {
		...
		for (let instance of fs.readdirSync(folder, options)) {
			...
			// don't try to load any non-JSON files encountered
			if (!instance.name.match(/[^.]\.json/)) continue;
...
const MD_COMMENT = `
<!-- THIS FILE WAS AUTOMATICALLY GENERATED BY BUILD.JS; DO NOT EDIT DIRECTLY -->
`;

function MDtoHTML(source, output) {
	const html = marked.parse(fs.readFileSync(source, "utf8"));
	fs.writeFileSync(output, `${MD_COMMENT}${html}`);
}
...
function make(print, omit, markdown, dry) {
	...
	if (markdown) {
		if (marked) {
			...
			// get the readme file in the main directory
			MDtoHTML("./README.md", "./src/html/README.html");

			// convert the other md files in the src folder
			const base = "./src/md/";
			for (let path of fs.readdirSync(base)) {
				const source = `${base}/${path}`;
				const output = source.replace(/md/g, "html");
				MDtoHTML(source, output);
			}

		} else {
			console.error("Module \"marked\" could not be required.");
		}
	}
}

const GIT_STATUS = /([ MTADRCU!?]{2}) ([^ ]+)( -> ([^ ]+))?\n/g;

function *gitStatus() {

	// arguments for git
	const args   = ["status", "-s", "src/json"];

	// invoke git for the information we want
	const stdout = spawnSync("git", args, {encoding: "utf8"}).stdout;

	// parse and yield to caller
	for (let match of stdout.matchAll(GIT_STATUS)) {
		yield {
			index    : match[1][0],
			working  : match[1][1],
			path     : match[4] ?? match[2],
			original : match[2],
			moved    : Boolean(match[4]),
		};
	}
}

const GIT_TAGS = new Set(["new", "changed"]);

function addTag(path, tag, omit, dry) {
	...
	if (dry) {
		console.log(tag[0].toUpperCase(), data.name, data.tags);
	} else {
...
function removeTag(path, tags, omit, dry) {
	...
	// we only want to modify files that have the tags in question
	if (Iter.any(data.tags, tag => tags.has(tag)))
		data.tags = data.tags.filter(tag => !tags.has(tag));
	else
		return;

	if (dry) {
		console.log("D", data.name, data.tags);
	} else {
		fs.writeFileSync(path, JSON.stringify(data, null, 2));
	}
}

function gitTags(omit=true, dry=false) {

	// including these in the default skip list as it's not a feature
	const skip = new Set(["src/json/definitions.json"]);

	for (let status of gitStatus()) {

		// we only want to load .json files and anything else
		if (!status.path.match(/[^.]\.json/)) continue;

		// don't read in these files because we don't want them
		if (skip.has(status.path)) continue;
		
		// whether the feature should be marked as new
		if (status.index == "A")
			skip.add(addTag(status.path, "new", omit, dry));

		// whether the feature should be marked as changed
		if (status.index == "R" || status.index == "M")
			skip.add(addTag(status.path, "changed", omit, dry));
	}
	...
	for (let namespace of namespaces) {
		...
		for (let instance of fs.readdirSync(folder, options)) {
			...
			// don't do anything to paths we saw in the last section
			if (skip.has(path)) continue;

			removeTag(path, GIT_TAGS, omit, dry);	
		}
	}

	if (!dry) spawnSync("git", ["add", "-p", "src/json"], {
		encoding : "utf8",
		stdio    : "inherit"
	});
}
...
function main(args) {
	...
	try {
		...
		parser.add_argument("-t", "--update-tags", {
			help   : "add/remove \"new\" and \"changed\" tags for those features",
			action : "store_true",
		});
